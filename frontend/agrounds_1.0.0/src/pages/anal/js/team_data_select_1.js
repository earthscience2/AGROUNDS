...
import '../css/team_data_select_1.scss';

// API
import { GetS3DataFilesApi, GetUserUploadFilesApi, GetTeamUploadFilesApi } from '../../../function/api/upload/uploadApi';
...
const Team_Data_Select_1 = () => {
  ...
  // 뒤로가기 함수 - 팀 정보 페이지로 이동
  const handleBack = () => {
    navigate('/app/team/info', { state: { activeTab: 'analysis' } });
  };

  // 팀 업로드 파일 목록 불러오기 (team_code로 Upload.user_code 조회)
  const fetchDataFiles = async (reset = true) => {
    try {
      ...
      // 팀 코드 가져오기
      const teamCode = sessionStorage.getItem('selectedTeamCode') || sessionStorage.getItem('teamCode') || localStorage.getItem('team_code');
      
      console.log('sessionStorage 확인:', {
        selectedTeamCode: sessionStorage.getItem('selectedTeamCode'),
        teamCode: sessionStorage.getItem('teamCode'),
        localStorageTeamCode: localStorage.getItem('team_code')
      });
      
      if (!teamCode) {
        console.warn('팀 코드가 없습니다.');
        setAllFiles([]);
        setDataFiles([]);
      ...
      console.log('팀 업로드 파일 조회 시작, team_code:', teamCode);
      const response = await GetTeamUploadFilesApi(teamCode);
      ...
      console.error('팀 업로드 파일 조회 오류:', error);
      setAllFiles([]);
      setDataFiles([]);
      setHasMoreData(false);
    } finally {
  ...
  if (loading) {
    return (
      <div className='team-data-select-1-page'>
        ...
        <div className="data-select-container">
          <div className="header">
            ...
            <div className="header-content">
              <h1 className="text-h2">팀 데이터 선택</h1>
    ...
    <div className='team-data-select-1-page'>
      ...
      <div className="data-select-container">
        <div className="header">
          ...
          <div className="header-content">
            <h1 className="text-h2">팀 데이터 분석</h1>
            <p className="subtitle text-body">분석할 데이터 파일을 선택하여<br />팀 경기 분석을 시작해보세요</p>
          </div>
        </div>
      </div>
      ...
      <div className="files-list">
        {dataFiles.length === 0 ? (
          <div className="empty-state">
            <p className="text-body">생성된 데이터 파일이 없습니다.</p>
            <button 
...
export default Team_Data_Select_1;
