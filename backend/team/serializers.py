...
class Team_Info_Serializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # validate team_name
        if not (2 <= len(data['team_name']) <= 15):
            raise serializers.ValidationError("team_name은 2글자 이상 15글자 이하로 설정해야 합니다.")
        if not all(char.isalnum() or char in "!@#$%^&*()_+-=<>?/.,;:[]{}" for char in data['team_name']):
            raise serializers.ValidationError("team_name은 영문, 한글, 숫자, 특수문자로만 구성되어야 합니다.")

        # validate team_host
        try:
            UserInfo.objects.get(user_code = data['team_host'])
        except UserInfo.DoesNotExist :
            raise serializers.ValidationError(f"유저코드 {data['team_host']}에 해당하는 선수 정보가 존재하지 않습니다.")
    ...
    def create(self, validated_data):
        ...
        uesr_code = validated_data['team_host']
        
        
        # == team_player 테이블에 row 추가 ==
